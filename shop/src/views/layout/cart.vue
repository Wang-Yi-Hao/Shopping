  ...
  <div class="cart">
    ...
    <div v-if="isLogin && cartList.length>0">
      ...
      <div class="cart-title">
        <span class="all">共<i>{{cartTotol}}</i>件商品</span>
...
export default {
  ...
  computed: {
    ...
    ...mapGetters('cart', ['cartTotol', 'selCaretList', 'selCount', 'selPrice', 'isAllChecked']),
    isLogin () {
    ...
    async goPay () {
      if (this.selCount > 0) {
        this.$router.push({
          ...
          query: {
            mode: 'cart',
            cartIds: this.selCaretList.map(item => item.id).join(',')
          }
        })
      }
    }
